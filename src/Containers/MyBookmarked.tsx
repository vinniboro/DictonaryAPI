...
import BackButton from "../BackButton";
  ...
  interface MyBookmarked {
    // other properties...
    vincentBookmark: bookmark[];
  }
  

const MyBookmarked: React.FC<MyBookmarked> = ({vincentBookmark}) => {
    return(
        <div className="contain-page">
                <div className="row-contain">
                {
                    vincentBookmark.map((bookmark) => (
                        <div className="grid-card">
                        <p style={{fontSize:'14px'}}>{bookmark.date}</p>
                        <h2>{bookmark.word}</h2> 
                        <p style={{height:'25vh'}}>{bookmark.explanation}</p>
                        <button>
